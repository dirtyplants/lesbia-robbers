...
% Wilson Society Guidelines: https://wilsonsociety.org/pubs/wjo/
...
%\corrauthor[1,3]{}{boris@uic,phil@south}
...
Many flower visitors engage in floral larceny, a suite of so-called `illegitimate' visits in which foragers take nectar without providing pollination services. %insert transition sentence here
...
%
% FIXME: make table 1 of terminology (each mode)
...
% Relatively quantifying the modes of nectar foraging could enable us to understand ...
% Further unanswered questions about tomia and whatnot ... evolutionary game stuff.
...
% FIXME: add contingency table for sex, ID, mode
...
%%%
Similarly, we recorded the species identifications, but found them to be implausible or uncertain for a number of individuals, and we consequently highlight the jointly presented feeding habits for all of \textit{Lesbia}. %until the taxonomy is clearer
...
% Zar: Ch. 23.6: "However, it is not strictly proper to test statistical hypotheses developed after examining the data to be tested. Therefore, the analysis of a subdivided contingency table should be considered only as a guide to developing hypotheses. Hypotheses suggested by this analysis then can be tested by obtaining a new set of data from the population of interest and stating those hypotheses in advance of the testing."

\subsection*{Bill morphology}

Short-billed hummingbirds and those with serrated tomia are thought to be more likely to nectar-rob flowers with long corolla tubes, co-adapted with long-billed hummingbirds \citep{lara2001}. %FIXME rephrase this
...
\noindent All accessions, metadata, image locations, and scored data used in our analyses are available online in Supplemental Materials. %FIXME: deposit on Dryad
\\
...
With a number of limitations in mind, we present a few preliminary analyses in an attempt to estimate the association of species ID, sex, and mode of visitation. %First, if we ignore species ID uncertainty and lump the observations from the at the generic level, we find that there is a trend of association between crudely scored larceny and sex (Fisher's Exact Test, p=0.06). Similarly, if we ignore sex, the (sometimes uncertain) species ID is weakly associated with with the occurrence of larceny (Fisher's Exact Test, p=0.05). %Clearly, these predictors may interact, so we also examined the association between variables species ID+sex, and crudely scored larceny with a 3-way contingency table (using the \textit{loglm} function in R package MASS; \citealt{R}).
...
%
%Our observations preliminary study indicates that nectar robbing comprises a surprisingly common mode of foraging in trainbearers. 
%
%First study that quantifies the mode of nectar foraging in hummingbirds
%
...
%
%Sex- and species-dependence (and altitude, unaccounted for predictors)
%
%The consequences for Ornelas' hypothesis are unclear. The sexual dimorphism is a solid argument, but even that is not a nail in the coffin, because males and females may have different foraging strategies. Further study needed.
